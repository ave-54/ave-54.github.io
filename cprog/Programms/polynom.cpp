...
#include "102-01.h"
void poly::load(int n0, double p[]){
	n=n0;				// закрытый метод загрузки массива
	pd=new double[n+1];	// - не всегда корректно вызывается
	for(int i=0;i<=n;i++)
		pd[i]=p[i]; }
void poly::extend(int n1){	// увеличение размерности полинома
	int i;
	if (n1<=n) return;
...
void poly::normalize(){	// нормализация - удаление лишних 0
	while(n>0 && pd[n]==0) n--;
	}					// память не перераспределяется
poly::poly(){			// "пустой" полином - нулевой степени
	n=0;				// с нулевым коэффициентом
	pd=new double[1];
	pd[0]=0;}
poly::poly(int m){		// полином заданной степени
	n=m;				// с нулевыми коэффициентами
	pd=new double[n+1];
	for(int i=0;i<=n;i++)
		pd[i]=0; }
poly::poly(int n0,double p[]){	// конструктор из массива коэффициентов
	load(n0,p); }
poly::poly(poly &T){	// конструктор "объект из объекта"
...
void poly::add(poly &T){	// Сложение - добавление второго к текущему
	extend(T.n);			// Увеличить до размерности второго
	for (int i=0;i<=T.n;i++) pd[i]+=T.pd[i];
	normalize();
	}
void poly::mul(poly &T){	// Умножение 
...
void poly::diff(){			// Производная от полинома (k*x^n)' = k*n*x^(n-1)
	for (int i=0;i<n;i++)
		pd[i]=pd[i+1]*(i+1);
	n--;
	}
int poly::compare(poly &T){	// Сравнение полиномов
	int k=0;
